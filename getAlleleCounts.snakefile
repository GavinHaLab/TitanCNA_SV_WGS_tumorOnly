...
rule tumCounts:
	input: 
		expand("results/titan/tumCounts/{tumor}/{tumor}.tumCounts.chr{chr}.txt", tumor=config["samples"], chr=CHRS),		
		expand("results/titan/tumCounts/{tumor}.tumCounts.txt", tumor=config["samples"])

rule getHETsites:
	input:
		lambda wildcards: config["samples"][wildcards.tumor]
	output:
